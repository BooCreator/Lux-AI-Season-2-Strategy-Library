...
class ActionsFabric:
    ...
    def buildReharge(self, count:int) -> bool:
        ...
        if not self.check(): return False
        if count > self.action_cost:
            self.actions.append(self.unit.robot.recharge(count+self.action_cost))
            self.energy_cost -= count+self.action_cost
            self.steps += count+self.action_cost if self.unit.isType(ROBOT_TYPE.LIGHT) else ceil(count/10)
            return True
        return False
    # = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
