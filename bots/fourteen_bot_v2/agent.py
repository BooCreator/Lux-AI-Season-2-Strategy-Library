...
from strategy.game.factory.no_limit_v2 import FactoryStrategy as NoLimit
...
def initStrategy(env_cfg:EnvConfig, strategy:dict)->DefaultStrategy:
    if strategy is None: return DefaultStrategy(env_cfg)
        ...
        early = strategy.get('early', BestEarly)
        game = strategy.get('game', DefaultGame)
        robot = strategy.get('robot', NextGenerationRobots)
