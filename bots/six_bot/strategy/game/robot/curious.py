            ...
            for robot in item.robots.values():
                ...
                if len(unit.action_queue) == 0:
                    ...
                    if robot.robot_task == RobotData.TASK.MINER:
                        ...
                        ct = findClosestTile(unit.pos, ice_map, lock_map=eyes.neg(eyes.sum([eyes.getFree(1) - ice_map, 'u_move'])))
                        ...
                        if onResourcePoint(unit.pos, ice_map) and unit.unit_id not in return_robots:
                            ...
                            # --- если могут задавить - отходим ---
                                ...
                                locked_field = np.where(eyes.sum(['factories', 'u_move', eyes.norm(eyes.diff(['e_energy', 'u_energy']))]) > 0, locked_field, 1)
                                points = []
                                ...
                                if len(points) > 0:
                                    if unit.power > move_cost + action_cost:
                                        ...
                                        eyes.update('u_move', unit.pos, 0)
                                        eyes.update('u_move', points[0], 1)
                                        robot.min_task += 1
                                    else:
                                        # --- иначе, копим энергию ---
                        ...
                        # --- если робот где-то гуляет ---
                            ...
                            locked_field = np.where(eyes.sum(['factories', 'u_move', norm]) > 0, 0, 1)
                            #eyes.log(['factories', 'u_move', 'e_energy', 'u_energy', locked_field],f'log/step/{player}')
                            points = []
                    ...
                    # --- если робот - чистильщик ---
                        ...
                        ct = findClosestTile(item.factory.pos, rubble_map, lock_map=eyes.update(eyes.neg('u_move'), unit.pos, 1)*rubble_map)
                        # --- если робот находится на блоке с фабрикой ---
                        if robot.on_position(item.factory.pos, size=3):
                        ...
                        # --- если робот где-то гуляет ---
                            ...
                            norm = eyes.norm(eyes.diff(['e_energy', 'u_energy']))
                            norm = np.where(norm < 0, 0, norm)
                            locked_field = np.where(eyes.sum(['factories', 'u_move', norm]) > 0, locked_field, 1)
                            points = []
                            # --- если робот - идёт на базу ---
                            if unit.unit_id in return_robots:
                                m_actions, move_cost, points = getMoveActions(game_state, unit, to=item.getNeareastPoint(unit.pos), locked_field=locked_field, has_points=True)
